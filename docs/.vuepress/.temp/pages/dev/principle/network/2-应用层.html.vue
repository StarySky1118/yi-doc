...
<p>HTTP 报文包括行、头、体三部分。</p>
<ol>
<li>对于 HTTP 请求报文，主要包括请求行、请求头和请求体。</li>
</ol>
<p>请求行包括请求方法、请求目标和 HTTP 协议版本。</p>
<p>请求头包含请求的附加信息，例如 User-Agent 等。</p>
<p>请求体包含请求的数据，例如 Post 请求可在请求体中放入数据。</p>
<ol start="2">
...
<code v-pre>503 Service Unavailable</code>：表示服务器当前无法提供服务。
<code v-pre>504 Gateway timeout</code>：表示网关获取后端请求超时。</p>
<h2 id="http-长连接是什么" tabindex="-1"><a class="header-anchor" href="#http-长连接是什么"><span>HTTP 长连接是什么？</span></a></h2>
<p>HTTP 是基于 TCP 的，因此客户端与服务端使用 HTTP 通信之前，首先要建立 TCP 连接。</p>
<p>HTTP 长连接是指，客户端与服务端在完成一次请求-响应之后，不释放 TCP 连接，而是继续在这个 TCP 连接上使用 HTTP 进行数据传输。
这样可以避免频繁建立与断开 TCP 连接造成的开销。</p>
<h2 id="http-为什么不安全" tabindex="-1"><a class="header-anchor" href="#http-为什么不安全"><span>HTTP 为什么不安全？</span></a></h2>
<p>由于 HTTP 使用明文传输，因此会带来窃听风险、篡改风险与冒充风险。</p>
<h2 id="https-与-http-有什么区别-https-是如何工作的" tabindex="-1"><a class="header-anchor" href="#https-与-http-有什么区别-https-是如何工作的"><span>HTTPS 与 HTTP 有什么区别？HTTPS 是如何工作的？</span></a></h2>
<p>HTTPS 在 HTTP 的基础上，在应用层和传输层添加 SSL/TLS 层，以实现数据加密传输。</p>
<p>HTTPS 首先通过 TLS 握手过程交换密钥，后续都基于该密钥进行对称加密传输。
密钥交换算法又分为 RSA 算法和 ECDHE 算法，RSA 是传统的密钥交换算法，ECDHE 才是大多数网站所使用的。</p>
<p>由于 ECDHE 算法是在 RSA 基础上改进而来的，因此我着重介绍基于 RSA 算法 TLS 握手过程。
其过程如下：</p>
...
<p>服务端接收到数据后，使用自己的私钥解密，获取 pre-master，组合成为最终密钥。</p>
<p>这样一来，客户端和服务端就通过协商安全交换了密钥，后续可以使用该密钥进行对称加密通信。</p>
<h2 id="客户端如何进行证书校验" tabindex="-1"><a class="header-anchor" href="#客户端如何进行证书校验"><span>客户端如何进行证书校验？</span></a></h2>
<p>证书一般分为两个部分：载荷和签名。
载荷部分包含服务端公钥、使用的哈希函数等信息，
签名的计算方法是首先做第一部分的哈希，然后使用 CA 私钥进行签名。</p>
<p>证书验证就是比对两个哈希的过程，第一个哈希通过 CA 公钥解密签名得到，第二个哈希通过对证书第一部分取相同哈希得到。
如果两个哈希相同，证书校验通过。</p>
<h2 id="什么是-https-中间人攻击-如何防范" tabindex="-1"><a class="header-anchor" href="#什么是-https-中间人攻击-如何防范"><span>什么是 HTTPS 中间人攻击？如何防范？</span></a></h2>
...
<h2 id="http-2-相比-http-1-1-有哪些改进" tabindex="-1"><a class="header-anchor" href="#http-2-相比-http-1-1-有哪些改进"><span>HTTP 2 相比 HTTP 1.1 有哪些改进？</span></a></h2>
<p>HTTP 2 相比 HTTP 1.1 的改进主要在提高传输效率、改进队头阻塞和服务器主动推送方面。</p>
<p>首先，HTTP 2 使用 HPACK 算法进行头部压缩，并全面采用二进制格式，这极大提高了传输效率。</p>
<p>HTTP 2 引入 Stream 概念，多个 Stream 共用 TCP 连接，解决了队头阻塞问题。</p>
<p>HTTP 2 中，服务端可以主动推送资源。</p>
<h2 id="dns-的端口号是多少" tabindex="-1"><a class="header-anchor" href="#dns-的端口号是多少"><span>DNS 的端口号是多少？</span></a></h2>
<p>53</p>
<h2 id="dns-是基于什么协议实现的" tabindex="-1"><a class="header-anchor" href="#dns-是基于什么协议实现的"><span>DNS 是基于什么协议实现的？</span></a></h2>
<p>DNS 是基于 UDP 协议实现的。</p>
...
<p>如果希望标识客户端，需要在请求中包含客户端信息，服务端才能够处理。</p>
...
服务端就可以利用这个 SessionId 查询 Session 数据，识别出客户端。</p>
...
<p>跨域问题是同源策略带来的问题。</p>
<p>同源是指两个 URL 协议、主机名、端口号都相同，同源策略要求网页只能加载、操作同源的资源。
