...
<h2 id="spring-bean-的生命周期" tabindex="-1"><a class="header-anchor" href="#spring-bean-的生命周期"><span>Spring Bean 的生命周期</span></a></h2>
<p>Spring Bean 的生命周期主要分为五个阶段：实例化、属性设置、初始化、使用、销毁。</p>
<p>实例化阶段使用无参构造方法创建这个 Bean 实例。</p>
<p>属性设置阶段会注入 Bean 的属性。例如使用 <code v-pre>@Autowire</code>注解或 <code v-pre>@Resource</code>注解注入。</p>
...
<p><code v-pre>@Service</code>标识了一个 Service 层 Bean。</p>
<p><code v-pre>@Repository</code>标识了一个 Dao 层 Bean。</p>
<p><code v-pre>@Transactional</code>是声明式事务管理注解。</p>
<h2 id="transactional注解失效的场景" tabindex="-1"><a class="header-anchor" href="#transactional注解失效的场景"><span><code v-pre>@Transactional</code>注解失效的场景</span></a></h2>
<p><code v-pre>@Transactional</code>是使用 Spring AOP 实现的，其基本原理是生成一个包含编程式事务处理的代理类来执行相应的方法。因此 <code v-pre>@Transactional</code>注解失效场景可分为两大类：第一类是底层实现原理本身带来的限制，第二类是使用不当导致失效。</p>
<p>第一类失效场景包括：</p>
<ol>
<li>无注解方法调用有注解方法，外部调用无注解方法。这会导致不使用代理类执行相应方法，从而失效。</li>
<li>注解方法非 <code v-pre>public</code>修饰，这也是底层实现原理的限制</li>
</ol>
<p>第二类失效场景包括：</p>
<ol>
<li><code v-pre>@Transactional</code>有一个关键属性 <code v-pre>rollBackFor</code>，这个属性标识了出现哪些异常时进行回滚。若此属性为默认值，默认行为是出现 <code v-pre>Unchecked Exception</code>时进行回滚。如果出现其他类型异常则不会回滚。</li>
<li>如果异常被捕获，没有抛出，也会导致失效</li>
...
<p>堆为了满足动态分配、动态回收的需求；
栈为了满足方法调用的需求，调用方法时压栈，方法返回时弹栈。</p>
...
<p>线程私有区域包括程序计数器、Java 虚拟机栈和本地方法栈。</p>
<ol>
<li>程序计数器</li>
</ol>
<p>简单来说，程序计数器 PC 指向字节码的某一行，字节码解释器通过程序计数器来依次执行指令，对代码解释执行。
多线程情况下，PC 用于指向当前线程执行的位置，从而进行线程切换。
程序计数器随着线程创建而创建，随着线程结束而消亡。</p>
<ol start="2">
<li>Java 虚拟机栈</li>
</ol>
<p>所有的 Java 方法调用都是通过 Java 虚拟机栈来实现的。
每进行一次方法调用，都会生成一个栈帧压栈，栈帧包括局部变量表、操作数栈、方法返回地址等。</p>
<p>线程公共区域包括堆和方法区。</p>
<ol>
<li>堆</li>
</ol>
<p>存放对象实例。堆是垃圾回收器管理的主要区域。堆中包含字符串常量池，这个区域可以复用字符串，避免字符串的重复创建。</p>
<ol start="2">
<li>方法区</li>
</ol>
...
<p>首先，Java 中的每个对象都有一个对象头，对象头保存了对象的运行时数据，例如锁信息、hashcode、GC 信息等。</p>
...
当轻量级锁竞争激烈时会升级为重量级锁，重量级锁使用操作系统的互斥锁实现，获取锁失败的线程进入阻塞状态。</p>
<p>总而言之，<code v-pre>synchronized</code> 关键字使用到对象头和监视器锁实现。</p>
...
<p>对象是否死亡，使用可达性分析算法进行判断。这个算法的思想是通过一系列 GC ROOTS 作为起点，通过引用链搜索对象，通过这些引用链不可达的对象就是死亡对象。
这些 GC ROOTS 是保证存活状态的对象，例如类静态字段指向的对象。</p>
<h3 id="垃圾回收算法" tabindex="-1"><a class="header-anchor" href="#垃圾回收算法"><span>垃圾回收算法</span></a></h3>
<p>Java 常见的垃圾回收算法包括：标记-清除、标记-整理、复制和分代收集算法。</p>
<h4 id="标记-清除" tabindex="-1"><a class="header-anchor" href="#标记-清除"><span>标记-清除</span></a></h4>
<p>首先标记出所有可达的对象，然后将不可达的对象清除。</p>
<p><strong>缺点：效率不高；产生大量内存碎片。</strong></p>
<h4 id="标记-整理" tabindex="-1"><a class="header-anchor" href="#标记-整理"><span>标记-整理</span></a></h4>
<p>首先标记所有存活的对象，然后将他们整理到堆的一端，然后清除掉边界外的内存。</p>
<p><strong>优点：解决了标记-清除算法内存碎片问题。</strong></p>
<p><strong>缺点：整理过程耗时。</strong></p>
<h4 id="复制" tabindex="-1"><a class="header-anchor" href="#复制"><span>复制</span></a></h4>
<p>将内存分为两部分，每次只使用其中一块，当一块用完时，将存活的对象复制到另一块，清除这一块。</p>
<p><strong>优点：比标记-整理算法效率高。</strong></p>
<p><strong>缺点：内存消耗加倍。</strong></p>
<h4 id="分代收集算法" tabindex="-1"><a class="header-anchor" href="#分代收集算法"><span>分代收集算法</span></a></h4>
<p>将内存分为新生代、老年代等，针对不同年代的特点采用不同的垃圾回收算法。</p>
<h3 id="java-垃圾收集器" tabindex="-1"><a class="header-anchor" href="#java-垃圾收集器"><span>Java 垃圾收集器</span></a></h3>
<p>JDK8 之前的默认垃圾收集器是 Parallel GC，JDK9 之后的默认垃圾收集器是 G1 GC。</p>
<h4 id="serial-收集器" tabindex="-1"><a class="header-anchor" href="#serial-收集器"><span>Serial 收集器</span></a></h4>
<p>使用单线程完成垃圾回收工作，并且执行垃圾回收时，停止所有工作线程。</p>
<p>新生代使用复制算法，老年代使用标记-整理算法。</p>
<p><img src="@source/dev/principle/img_1.png" alt="img_1.png"></p>
<p>Serial 收集器实现简单，开销低，很适合单核或小堆内存环境。</p>
<h4 id="parallel-收集器" tabindex="-1"><a class="header-anchor" href="#parallel-收集器"><span>Parallel 收集器</span></a></h4>
<p>Serial 收集器的多线程版本，注重吞吐量，即 CPU 用于执行用户代码的时间与 CPU 执行代码的总时间。</p>
<p><img src="@source/dev/principle/img_2.png" alt="img_2.png"></p>
<p>在 JDK8 中默认使用 Parallel 收集器，新生代使用复制算法，老年代使用标记-整理算法。</p>
<h4 id="cms-收集器" tabindex="-1"><a class="header-anchor" href="#cms-收集器"><span>CMS 收集器</span></a></h4>
<p>Concurrent Mark Sweep，并发标记清除。</p>
<p>算法流程：首先暂停所有工作线程，拍摄堆快照，获取 GC ROOTS；同时运行工作线程和 GC 线程，进行对象可达性分析；再次暂停所有工作线程，拍摄堆快照，更新可达性；同时开启工作线程和 GC 线程，启动清除工作。</p>
<p>缺点：标记-清除算法会产生大量内存碎片。因此它在 JDK9 中标记为过时，在 JDK14 中被清除。</p>
<h4 id="g1-收集器" tabindex="-1"><a class="header-anchor" href="#g1-收集器"><span>G1 收集器</span></a></h4>
...
<ol>
<li>及时释放资源</li>
<li>使用软引用或弱引用指向可能长期不使用的对象</li>
<li>避免使用静态集合持有短生命周期对象</li>
</ol>
<h2 id="threadlocal-的原理" tabindex="-1"><a class="header-anchor" href="#threadlocal-的原理"><span>ThreadLocal 的原理</span></a></h2>
<p>ThreadLocal 类型的变量在每个线程中都有独特的副本。
其实现原理是在每个 <code v-pre>Thread</code>类中都有一个 <code v-pre>ThreadLocalMap</code>，这个 <code v-pre>ThreadLocalMap</code>是一个哈希表，哈希表的键是 <code v-pre>ThreadLocal</code>对象的引用，值是线程独有的变量值副本。可以使用 <code v-pre>ThreadLocal</code>的 <code v-pre>set()</code>和 <code v-pre>get()</code>方法修改获取变量值副本。
<code v-pre>ThrealLocalMap</code>对 <code v-pre>ThreadLocal</code>对象的引用是弱引用，如果 <code v-pre>ThreadLocal</code>对象被回收，但 <code v-pre>ThreadLocalMap</code>值对象未被回收，那么就无法通过键获取变量值，导致内存泄漏问题。
