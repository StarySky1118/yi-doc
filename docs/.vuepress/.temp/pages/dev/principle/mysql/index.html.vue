...
<p>MyISAM 存储引擎不支持事务，InnoDB 存储引擎支持四种隔离级别的事务。</p>
<p>为实现事务的隔离性，InnoDB 存储引擎使用 MVCC 和行级锁机制；MyISAM 存储引擎不支持行级锁。</p>
<p>为实现事务的持久性，InnoDB 存储引擎使用 Redo Log 机制，支持崩溃恢复；MyISAM 不支持崩溃恢复。</p>
<p><strong>索引</strong></p>
<p>二者索引采用的数据结构都为 B+ 树。不同之处在于，InnoDB 存储引擎 B+ 树叶子结点存储真实数据，
MyISAM 存储引擎 B+ 树只存放索引，数据独立存储。</p>
...
<p>执行器会将执行计划发送给存储引擎，获取结果并返回给客户端。</p>
</div></template>
